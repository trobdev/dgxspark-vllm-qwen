...
The advantage here is **capacity, not raw bandwidth.** The unified pool runs at ~273 GB/s (LPDDR5X) — modest next to a discrete GPU's VRAM (an H100 moves >3 TB/s). What the Spark buys you is the ability to hold a 35B model *and* a large KV cache in one pool at all, on a single device, without the multi-GPU rack that capacity would otherwise demand. You trade peak throughput for the fact that the workload fits — which is why generation lands at ~60–80 tokens/second (see [Throughput Expectations](#throughput-expectations-and-limitations)) rather than cloud-cluster speeds.
...
**Ngram speculative decoding was removed** — `--spec-method ngram --spec-tokens 5` was present in earlier builds but crashed vLLM's `EngineCore` with a KV-cache block accounting assertion on large contexts. The compose file now omits these flags; speculative decoding is re-enabled on a future vLLM release once the bug is fixed.
...
**Speculative decoding** (planned: ngram, 5 tokens) exploits a property of code generation: code is highly repetitive and locally predictable. Common patterns (`return None`, `self.`, `import `, variable names that just appeared) can be predicted by simply matching n-grams in the existing context. vLLM's ngram decoder proposes 5 candidate tokens at a time based on pattern matching, then verifies them all in a single forward pass. When the predictions are correct (frequently, for boilerplate code), you get 5 tokens for the cost of 1 forward pass — a direct throughput multiplier.

> **Note:** ngram speculative decoding was removed in a later vLLM build due to a KV-cache block assertion crash. It will be re-enabled on a future vLLM release.
...
**Throughput (tokens/second):** Expect 60–80 tokens/second on typical coding responses — fast enough that a 500-token response streams in under 10 seconds. (Ngram speculative decoding was removed due to a vLLM 0.22.1rc1 crash on large contexts; re-enabling it on a stable release would push this higher.)
